...
class UpdateTillIDOrderController extends GetxController {
  final UpdateTillIdOrderRepository _updateOrderRepository = UpdateTillIdOrderRepository();
  ...
  Future<void> updateOrderByTillID(
    ...
    try {
      ...
      UpadateTillIdOderModel response = await _updateOrderRepository.updateOrderByTillID(
        orderId, customerId, totalBags, weight, totalPrice,
      );

      updateResponse.value = response;

      print("✅ Update Successful: ${response.title}");
      Get.snackbar("Success", response.title ?? "Order updated successfully!", snackPosition: SnackPosition.BOTTOM);
    } catch (error) {
